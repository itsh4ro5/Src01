...
async def handler(event):
    ...
    # 1. HUMAN SLEEP CHECK
    if IS_PAUSED:
        await event.reply("⏳ **Bot abhi thodi der ka break (rest) le raha hai taaki account safe rahe. Kripya 15-20 minute baad try karein.**")
        return
    ...
    try:
        # 2. RANDOM DECIMAL DELAY (Human behavior simulate karne ke liye)
        delay_time = random.uniform(2.1, 4.5)
